...
三张表（都按 `script_id` + `is_deleted=0` 过滤，系统自动注入）：

| collection | 用途 | 关键字段 |
|---|---|---|
| `seca_scene_analysis` | 场景摘要 | scene_sort / scene_title / scene_brief |
...
> ⚠️ History 存内存：联调期固定**单进程**、勿改后端代码（`--reload` 改代码会清空内存 history，`--workers>1` 进程不共享）。
...
```bash
# 离线纯逻辑（48 用例：guard/budget/formatter/compactor/history/agent-loop-mock）
...
| `scripts/test_mongo_connection.py` | 连接 + 三表字段结构 | Mongo |
...
真实测试默认剧本：`script_id=690c1b6736c9c50c40160976`（肖申克的救赎，三表齐全、单版本）。
...
1. 单结果太大 → tool budget 硬截断 `[TOOL_RESULT_TRUNCATED]`（字段级 `[FIELD_TRUNCATED]`）
2. 单次提问查太多轮 → `MAX_TOOL_ROUNDS=4` + 终答 `tool_choice=none`
3. session 历史堆积 → 旧 tool 消息压缩 `[TOOL_RESULT_COMPRESSED]`（保留最近 `KEEP_LATEST_TOOL_MESSAGES=4`）
